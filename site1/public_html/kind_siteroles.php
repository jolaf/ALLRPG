...
if($orders==1) {
	...
	if($id>0) {
		if($showonlyacceptedroles) {
			$result=mysql_query("SELECT * FROM ".$prefix."roles where id=".$id." and todelete!=1 and todelete2!=1 and status=3 and site_id=".$subobj);
		}
		else {
			$result=mysql_query("SELECT * FROM ".$prefix."roles where id=".$id." and todelete!=1 and todelete2!=1 and site_id=".$subobj);
		}
		$a = mysql_fetch_array($result);
		...
		if($a["id"]!='' && $d["rights"]!=1) {
			...
			$pagetitle=h1line(decode($c["title"]).' – <a href="'.$server_absolute_path.$kind.'/'.$c["id"].'/orders=1">заявки</a>',$curdir.$kind.'/'.$subobj.'/');

			$content2.='<h1>'.decode($a["sorter"]).'</h1>';
			if(($b[0]<$f["kolvo"] && $c["status2"]==2 && $c["testing"]!='1') || ($b[0]>1 && $d["rights"]!=1)) {
				if($b[0]<$f["kolvo"] && $c["status2"]==2 && $c["testing"]!='1') {
					if($_SESSION["user_id"]!='') {
						$additional_commands.='<a href="'.$server_absolute_path.'order/act=add&subobj='.$subobj.'&roletype='.$f["team"].'&wantrole='.$f["id"].'">';
					}
					else {
						$additional_commands.='<a href="'.$server_absolute_path.'register/redirectobj=order&redirectid='.$subobj.'&redirectparams=roletype:'.$f["team"].'*wantrole:'.$f["id"].'">';
					}
					$additional_commands.='Подать заявку на ту же роль</a>';
				}
				if($b[0]>1 && $d["rights"]!=1) {
					$additional_commands.='<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&wantrole='.$f["id"].'">Поданные на ту же роль заявки</a>';
				}
			}
			...
			$content2.='<br>';
			if($a["vacancy"]!='' && $f["name"]!='') {
				$content2.='<b>Роль</b>: <a href="'.$server_absolute_path.$kind.'/'.$subobj.'/'.$f["id"].'/">'.decode($f["name"]).'</a><br>';
			}
			...
			for($i=0;$i<count($rolefields);$i++) {
				if($rolefields[$i]["read"]==1 && (decode($allvalues[$rolefields[$i]["name"]])!='' || $rolefields[$i]["type"]=='h1' || $rolefields[$i]["type"]=='checkbox')) {
					if($rolefields[$i]["type"]=='h1') {
						$content2.='<h1>'.$rolefields[$i]["sname"].'</h1>';
					}
					else {
		...
		if($havelocats) {
			if(encode($_GET["wantrole"])!='') {
				$query="SELECT t1.*, t2.name as locatname, t2.id as locatid, t3.sid, t3.nick, t3.fio, t3.hidesome, t4.name as vacancyname FROM ".$prefix."users t3, ".$prefix."roles t1 LEFT JOIN ".$prefix."roleslocat t2 ON t2.id=t1.locat LEFT JOIN ".$prefix."rolevacancy t4 ON t4.id=t1.vacancy WHERE (t2.rights!=1 || t1.locat='') and t1.todelete!=1 and t1.todelete2!=1 and t3.id=t1.player_id and t4.id=".encode($_GET["wantrole"])." and t1.site_id=".$subobj." order by FIELD(t2.id, ".$alllocats_ids.")";
			...
			elseif(encode($_GET["wantrole2"])!='') {
				$query="SELECT t1.*, t2.name as locatname, t2.id as locatid, t3.sid, t3.nick, t3.fio, t3.hidesome, t4.name as vacancyname FROM ".$prefix."users t3, ".$prefix."roles t1 LEFT JOIN ".$prefix."roleslocat t2 ON t2.id=t1.locat LEFT JOIN ".$prefix."rolevacancy t4 ON t4.id=t1.vacancy WHERE (t2.rights!=1 || t1.locat='') and t1.todelete!=1 and t1.todelete2!=1 and t3.id=t1.player_id and t4.id=".encode($_GET["wantrole2"])." and t1.status=3 and t1.site_id=".$subobj." order by FIELD(t2.id, ".$alllocats_ids.")";
				...
				$query="SELECT t1.*, t2.name as locatname, t2.id as locatid, t3.sid, t3.nick, t3.fio, t3.hidesome, t4.name as vacancyname FROM ".$prefix."users t3, ".$prefix."roles t1 LEFT JOIN ".$prefix."roleslocat t2 ON t2.id=t1.locat LEFT JOIN ".$prefix."rolevacancy t4 ON t4.id=t1.vacancy WHERE (t2.rights!=1 || t1.locat='') and t1.todelete!=1 and t1.todelete2!=1 and t3.id=t1.player_id and t1.site_id=".$subobj." order by FIELD(t2.id, ".$alllocats_ids.")";

				$query2="SELECT COUNT(t1.id) FROM ".$prefix."users t3, ".$prefix."roles t1 LEFT JOIN ".$prefix."roleslocat t2 ON t2.id=t1.locat LEFT JOIN ".$prefix."rolevacancy t4 ON t4.id=t1.vacancy WHERE (t2.rights!=1 || t1.locat='') and t1.todelete!=1 and t1.todelete2!=1 and t3.id=t1.player_id and t1.site_id=".$subobj;
			}
			if($sorting==1) {
				$query.=', '.$ordfield.' ASC';
			}
			...
			elseif(encode($_GET["wantrole2"])!='') {
				...
				$query="SELECT t1.*, t2.name as locatname, t2.id as locatid, t3.sid, t3.nick, t3.fio, t3.hidesome, t4.name as vacancyname FROM ".$prefix."users t3, ".$prefix."roles t1 LEFT JOIN ".$prefix."roleslocat t2 ON t2.id=t1.locat LEFT JOIN ".$prefix."rolevacancy t4 ON t4.id=t1.vacancy WHERE (t2.rights!=1 || t1.locat='')  and t1.todelete!=1 and t1.todelete2!=1 and t3.id=t1.player_id and t1.site_id=".$subobj." order by ";
		...
		$pagetitle=h1line(decode($b["title"]).' – заявки',$curdir.$kind.'/'.$subobj.'/orders=1');

		$content2.='<center><div id="cb_editor">';
		$result2=mysql_query("SELECT COUNT(id) FROM ".$prefix."rolevacancy WHERE site_id=".$subobj);
		$b=mysql_fetch_array($result2);
		if($b[0]>0) {
			$additional_commands.='<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/">К сетке ролей</a>';
		}
		if(encode($_GET["wantrole"])!='' || encode($_GET["wantrole2"])!='') {
			$additional_commands.='<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1">Все заявки</a>';
		}
		...
		<tr valign="bottom">
		<td>
		<div class="siteroles total"><b>Всего заявок</b>: '.$b[0].'</div>
		<table class="menutable">
		<tr class="menu">';
		$content2.='
		<td><a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&sorting=';
		if($sorting==1) {
			$content2.='2" title="[отсортировать : игрок : по убыванию]" class="arrow_up">Игрок</a>';
		}
		elseif($sorting==2) {
			$content2.='1" title="[отсортировать : игрок : по возрастанию]" class="arrow_down">Игрок</a>';
		}
		else {
			$content2.='2" title="[отсортировать : игрок : по убыванию]">Игрок</a>';
		}
		$content2.='</td>';
		...
		<td><a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&sorting=';
		if($sorting==3) {
			$content2.='4" title="[отсортировать : '.strtolower($sorter).' : по убыванию]" class="arrow_up">'.$sorter.'</a>';
		}
		elseif($sorting==4) {
			$content2.='3" title="[отсортировать : '.strtolower($sorter).' : по возрастанию]" class="arrow_down">'.$sorter.'</a>';
		}
		else {
			$content2.='4" title="[отсортировать : '.strtolower($sorter).' : по убыванию]">'.$sorter.'</a>';
		}
		$content2.='</td>';

		$content2.='
		<td style="width:20%"><a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&sorting=';
		if($sorting==5) {
			$content2.='6" title="[отсортировать : статус : по убыванию]" class="arrow_up">Статус</a>';
		}
		elseif($sorting==6) {
			$content2.='5" title="[отсортировать : статус : по возрастанию]" class="arrow_down">Статус</a>';
		}
		else {
			$content2.='6" title="[отсортировать : статус : по убыванию]">Статус</a>';
		}
		$content2.='</td>';
		...
		if($kolvovacancy>0) {
			...
			<td><a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&sorting=';
			if($sorting==7) {
				$content2.='8" title="[отсортировать : роль : по убыванию]" class="arrow_up">Роль</a>';
			}
			elseif($sorting==8) {
				$content2.='7" title="[отсортировать : роль : по возрастанию]" class="arrow_down">Роль</a>';
			}
			else {
				$content2.='8" title="[отсортировать : роль : по убыванию]">Роль</a>';
			}
			$content2.='</td>';
		}
		...
		while($a=mysql_fetch_array($result)) {
			if(!$showonlyacceptedroles || $a["status"]==3) {
				...
				if($prevlocatid!=$a["locatid"] && $havelocats) {
					...
					$content2.='
			<tr class="string'.$string.'"><td class="locations" colspan=4>';
					if($a["locatid"]==0) {
				...
				$string=$string==1?2:1;
				$content2.='
			<tr class="string'.$string.'">
			<td>
			<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/'.$a["id"].'/orders=1">';
				if($a["status"]==4) {
					$content2.='<s>';
				}
	...
	// сетка ролей
	if($id>0) {
		$result=mysql_query("SELECT * FROM ".$prefix."rolevacancy where id=".$id." and site_id=".$subobj);
		$a = mysql_fetch_array($result);
		if($a["id"]!='') {
			...
			$pagetitle=h1line(decode($c["title"]).' – сетка ролей',$curdir.$kind.'/'.$subobj.'/');

			$content2.='<div id="cb_editor"><h1>'.decode($a["name"]).'</h1>';
			if(($e[0]<$a["kolvo"] && $c["status2"]==2 && $c["testing"]!='1') || ($b[0]>0 && $d["rights"]!=1) || ($e[0]>0 && $d["rights"]!=1)) {
				if($e[0]<$a["kolvo"] && $c["status2"]==2 && $c["testing"]!='1') {
					if($_SESSION["user_id"]!='') {
						$additional_commands.='<a href="'.$server_absolute_path.'order/act=add&subobj='.$subobj.'&roletype='.$a["team"].'&wantrole='.$id.'">';
					}
					else {
						$additional_commands.='<a href="'.$server_absolute_path.'register/redirectobj=order&redirectid='.$subobj.'&redirectparams=roletype:'.$a["team"].'*wantrole:'.$id.'">';
					}
					$additional_commands.='Подать заявку на данную роль</a>';
				}
				if($b[0]>0 && $d["rights"]!=1 && !$showonlyacceptedroles) {
					$additional_commands.='<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&wantrole='.$id.'">Поданные на роль заявки</a>';
				}
				if($e[0]>0 && $d["rights"]!=1) {
					$additional_commands.='<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/orders=1&wantrole2='.$id.'">Принятые на роль заявки</a>';
				}
			}
			if($a["locat"]!='' && $d["name"]!='' && $havelocats) {
				$content2.='<b>Локация / команда</b>: '.locatpath($d["id"]).'<br>';
			...
			$content2.='<b>Желаемое количество заявок</b>: '.decode($a["kolvo"]).'<br>';
			if(decode($a["content"])!='') {
		...
		if($a["id"]!='' && $b[0]>0) {
			if($havelocats) {
				if(encode($_REQUEST["locat"])!='') {
					$query="SELECT t1.*, t2.name as locatname, t2.id as locatid FROM ".$prefix."roleslocat t2 LEFT JOIN ".$prefix."rolevacancy t1 ON t2.id=t1.locat WHERE t2.site_id=".$subobj." AND (t2.description!='' OR t1.id!='') AND t2.id=".encode($_REQUEST["locat"])." order by t1.code ASC, t1.name ASC";
				}
				else {
					$query="SELECT t1.*, t2.name as locatname, t2.id as locatid FROM ".$prefix."roleslocat t2 LEFT JOIN ".$prefix."rolevacancy t1 ON t2.id=t1.locat WHERE t2.site_id=".$subobj." AND (t2.description!='' OR t1.id!='') order by FIELD(t2.id, ".$alllocats_ids."), t1.code ASC, t1.name ASC";
				}
			}
			else {
			...
			$pagetitle=h1line(decode($a["title"]).' – cетка ролей',$curdir.$kind.'/'.$subobj.'/');
			$content2.='<center>';
			$result2=mysql_query("SELECT COUNT(id) FROM ".$prefix."roles WHERE site_id=".$subobj." and todelete!=1 and todelete2!=1");
			$b=mysql_fetch_array($result2);
			if($b[0]>0) {
				$additional_commands.='<a href="'.$server_absolute_path;
				if($siteroles2) {
					$additional_commands.='siteroles2';
				}
				else {
					$additional_commands.=$kind;
				}
				$additional_commands.='/'.$subobj.'/';
				if(!$siteroles2) {
					$additional_commands.='orders=1';
				}
				$additional_commands.='">Поданные заявки</a>';
			}
			$content2.='
			<table class="menutable">
			<tr class="menu">
			<td>Роль</td>
			<td class="menu_description">Описание</td>
			<td colspan=2>Игроки</td>
			</tr>';
			...
			<tr class="string'.$string.'"><td class="locations" colspan=4>';
					...
					if($a["locatid"]!=0) {
						...
						if(decode($c["description"])!='') {
							$content2.='<tr class="string'.$string.'"><td colspan=4 class="description">'.decode2($c["description"]).'</td></tr>';
						}
					}
				}
				$rows=1;
				...
				<a href="'.$server_absolute_path.$kind.'/'.$subobj.'/'.$a["id"].'/">'.decode($a["name"]).$teamkolvo.'</a>
				</td>';
					if(decode($a["content"])!='') {
						$content2.='
				...
				'.decode($a["content"]).'
					...
					if($b[0]<$a["kolvo"] && !$orderclosed) {
						if($_SESSION["user_id"]) {
							$content2.='<td colspan=2><a href="'.$server_absolute_path.'order/act=add&subobj='.$subobj.'&roletype='.$a["team"].'&wantrole='.$a["id"].'">';
						}
						else {
							$content2.='<td colspan=2><a href="'.$server_absolute_path.'register/redirectobj=order&redirectid='.$subobj.'&redirectparams=roletype:'.$a["team"].'*wantrole:'.$a["id"].'">';
						}
					...
					if($c["rights"]==1) {
	                    if($b[0]+count($taken)>0) {
	                    	if($newstring) {
	                    		$content2.='<tr class="string'.$string.'">';
	                    	}
