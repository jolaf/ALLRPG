<?php
if($_SESSION["user_id"]=="") {
	redirect($server_absolute_path.'register/redirectobj=hosting');
}
if($_SESSION["user_id"]!='') {
	//заявки на хостинг
	...
	if($_SESSION["user_id"]!='') {
		$obj_r=new netRight(
			...
			false,
			100,
			'author='.$_SESSION["user_id"].' and usetemp=0',
			'author='.$_SESSION["user_id"].' and usetemp=0',
			'author='.$_SESSION["user_id"].' and usetemp=0'
		);
		$obj->setRight($obj_r);
	...
	$pagetitle=h1line('Открыть систему заявок и сайт',$curdir.$kind.'/');
