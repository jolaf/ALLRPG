...
if($_SESSION["user_id"]!='' /*&& $workrights["site"]["report"]*/) {
  ...
  $site_id = intval($_SESSION['siteid']);
	...
	$result = mysql_query ("
    SELECT rl.id, rl.parent, rl.name, COUNT(r.id) AS total_count, SUM(IF(r.status=1, 1, 0)) AS podana, SUM(IF(r.status=2, 1, 0)) AS inprocess, SUM(IF(r.status=3, 1, 0)) AS accepted, SUM(IF (r.alltold = 1, 0, 1)) AS alltold, SUM(IF (r.moneydone = 1, 1,0)) AS moneydone
    FROM  (
    ...
    WHERE r.id IS NULL OR (r.site_id = $site_id AND r.todelete = 0 AND r.todelete2 = 0 AND r.status<>4)
